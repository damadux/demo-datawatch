...
#define dw_untagged_addr(addr)  sign_extend64(addr, 47)
#define canonicalize_user_addr(addr) \
	(void *)dw_untagged_addr((unsigned long __force)addr)
...
({												\
	if(((__u64)addr & X86_TBMASK) != 0) {							\
		printk("1- Untagged usr addr : %p\n", addr;					\
		printk("2- Untagged usr addr : %p\n", canonicalize_user_addr(addr));		\
		printk("3- Untagged usr addr : %p\n", (__u64)canonicalize_user_addr(addr));	\
	}											\
	WARN_ON_IN_IRQ();									\
	likely(!__range_not_ok(canonicalize_user_addr(addr), size, TASK_SIZE_MAX));		\
})
